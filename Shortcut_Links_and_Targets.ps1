...
Here is a useful 1-liner if you only need information about a single shortcut file:
#$shortcutPath = "C:\Path\To\Your\Shortcut.lnk"; $wsh = New-Object -ComObject WScript.Shell; $sf = Get-Item $shortcutPath; $sc = $wsh.CreateShortcut($sf.FullName); $t = $sc.TargetPath; $a = $sc.Arguments; $te = $t -and (Test-Path $t); $ti = if ($te) { Get-Item $t } else { $null }; $szi = try { $c = Get-Content $sf.FullName -Stream Zone.Identifier -EA SilentlyContinue; $z = '-'; $r = '-'; $h = '-'; if ($c) { $c | % { if ($_ -match '^ZoneId=(\d+)') { $z = $matches[1] }; if ($_ -match '^ReferrerUrl=(.+)') { $r = $matches[1] }; if ($_ -match '^HostUrl=(.+)') { $h = $matches[1] } } }; @{ZoneId=$z;ReferrerUrl=$r;HostUrl=$h} } catch { @{ZoneId='-';ReferrerUrl='-';HostUrl='-'} }; $tzi = if ($te) { try { $c = Get-Content $t -Stream Zone.Identifier -EA SilentlyContinue; $z = '-'; $r = '-'; $h = '-'; if ($c) { $c | % { if ($_ -match '^ZoneId=(\d+)') { $z = $matches[1] }; if ($_ -match '^ReferrerUrl=(.+)') { $r = $matches[1] }; if ($_ -match '^HostUrl=(.+)') { $h = $matches[1] } } }; @{ZoneId=$z;ReferrerUrl=$r;HostUrl=$h} } catch { @{ZoneId='-';ReferrerUrl='-';HostUrl='-'} } } else { @{ZoneId='-';ReferrerUrl='-';HostUrl='-'} }; [PSCustomObject]@{ShortCut=$sf.FullName;TargetFile=$t;Arguments=$a;ShortCutSize=if($sf.Length){"{0:N2} KB"-f($sf.Length/1KB)}else{"-"};ShortcutSHA256=(Get-FileHash $sf.FullName -Algorithm SHA256 -EA SilentlyContinue).Hash;ShortCutOwner=try{(Get-Acl $sf.FullName).Owner}catch{"-"};ShortCutCreationTime=$sf.CreationTime;ShortCutLastWriteTime=$sf.LastWriteTime;ShortCutLastAccessTime=$sf.LastAccessTime;ShortcutZoneId=$szi.ZoneId;ShortcutReferrerUrl=$szi.ReferrerUrl;ShortcutHostUrl=$szi.HostUrl;TargetSize=if($ti){"{0:N2} KB"-f($ti.Length/1KB)}else{"-"};TargetExists=$te;TargetSHA256=if($te){(Get-FileHash $t -Algorithm SHA256 -EA SilentlyContinue).Hash}else{"-"};TargetOwner=if($te){try{(Get-Acl $t).Owner}catch{"-"}}else{"-"};TargetType=if($te){if(Test-Path $t -PathType Container){"Directory"}else{"File"}}else{"-"};TargetCreationTime=if($ti){$ti.CreationTime}else{"-"};TargetLastWriteTime=if($ti){$ti.LastWriteTime}else{"-"};TargetLastAccessTime=if($ti){$ti.LastAccessTime}else{"-"};TargetZoneId=$tzi.ZoneId;TargetReferrerUrl=$tzi.ReferrerUrl;TargetHostUrl=$tzi.HostUrl}
