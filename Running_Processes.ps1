...
.DESCRIPTION
This script gets information about all running processes. Results are written to a CSV.
...
.LINK
https://github.com/soc-otter/Blue/blob/main/Running_Processes.ps1

.EXAMPLE
PS> .\Running_Processes.ps1
#>

$outputPath = "C:\BlueTeam"
$outputFile = Join-Path $outputPath "Running_Processes.csv"
...
function Get-ProcessDetails {
    ...
    $allProcesses | ForEach-Object {
        ...
        $orphan = if ($currentProcess.ParentProcessID -notin $processIds -and $currentProcess.ParentProcessID -ne 0) { $true } else { $false }

        $owner = "-"
        try {
            $ownerResult = $currentProcess.GetOwner()
...
Write-Progress -Activity "Initializing" -Status "Starting process analysis" -PercentComplete 0
